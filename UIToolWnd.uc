...
function OnClickButton( String a_ButtonID )
{
	switch( a_ButtonID )
	{
		case "initBtn":
			...
			break;
		case "transBtn":
			if( transBtn.GetButtonName() == "투명 끔" )
			{
				ExecuteCommand("//투명 끔");
				transBtn.SetNameText( "투명 켬" );
			}
			else
			{
				ExecuteCommand("//hide on");
				transBtn.SetNameText( "투명 끔" );
			}
			break;
		...
		case "skillBtn":			
			ExecuteCommand("//setskill 7029 4");
			break;
		...
		case "weightBtn":
			if( weightBtn.GetButtonName() == "무게 끔" )
			{
				ExecuteCommand("//무게 켬");
				weightBtn.SetNameText( "무게 켬" );
			}
			else
			{
				ExecuteCommand("//무게 끔");
				weightBtn.SetNameText( "무게 끔" );
			}
			break;	
...
function linkHomePage()
{
	...
	//dsScript.ShowDialog( DIALOG_Modalless, DIALOG_Warning, "리니지2 홈페이지로 이동하겠습니다. 계속하시겠습니까?", "UIToolWnd" );
}
*/
